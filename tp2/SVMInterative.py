...
while i < 100: #numero de vezes que o modelo é medido.
    ...
    predictSVM = pd.DataFrame(svm.predict(X_test), columns=['PSS_Stress'])
    roundSVM = np.around(predictSVM)
    approxSVM.append(accuracy_score(y_test, roundSVM))
    
mediaKNN = np.mean(approxSVM)
print(mediaKNN)